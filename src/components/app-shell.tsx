      ...
      <NavBarMenuProvider>
        ...
        <div className={`flex min-h-0 flex-1 flex-col ${isLanding ? "pt-[var(--topbar-h)]" : ""}`}>
          ...
          >
            ...
            <SidebarInset
              className="min-h-0 flex-1 m-5 mt-0 mb-0 max-sm:m-0 bg-transparent max-sm:bg-sidebar"
              // Landing: the top AND bottom margins ease from m-5 (1.25rem) on
              // the splash down to 0 as you scroll into the Box product,
              // driven by --enter-progress (0→1) — same signal as the nav
              // reveal. Non-landing keeps the static mt-0/mb-0 so the card
              // sits flush against both the nav above and the footer below on
              // every page, landing or not.
              style={
                isLanding
                  ? {
                      marginTop: "calc(1.25rem * (1 - var(--enter-progress, 0)))",
                      marginBottom: "calc(1.25rem * (1 - var(--enter-progress, 0)))",
                    }
                  : undefined
              }
            >

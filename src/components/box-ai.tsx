...
export function BoxAI({
  ...
  const chatCard = (
    <ContentCard className="relative flex h-full w-full min-w-0 flex-col max-sm:pt-8 max-sm:pb-0 max-sm:[@media(display-mode:standalone)]:pb-0">
      {!embedded && (showTrigger || openCaseStudy || activeId) && (
        <div className="flex items-center gap-1 p-2">
          ...
          <button
            ...
            aria-label="Settings"
            className="sm:hidden absolute right-6 top-8 max-sm:[@media(display-mode:standalone)]:top-20 z-30 transition-colors active:scale-95"
          >
      ...
      {!embedded && !showTrigger && !openCaseStudy && !activeId && (
        <div className="sm:hidden absolute right-6 top-8 max-sm:[@media(display-mode:standalone)]:top-20 z-30">
          <button
            type="button"
            onClick={() => setSettingsOpen(true)}
            aria-label="Settings"

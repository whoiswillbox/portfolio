...
Variants, ApiNotes, PropsTable, Accessibility, Changelog).
...
- `Playground` — configurable controls (`select` / `boolean` / `text`) above a
  live render; pass a `render(values)` fn.
...
- `Install`, `PropsTable` (+ `PropInterface` groups), `ApiNotes`, `Specs`.
- `Accessibility` (Keyboard + ARIA tables, plus labeling / screen-reader /
  reduced-motion prose), `WcagChecklist` (contrast + criteria rows).
- `Changelog`.

`Related` still exists in the scaffold but has been **removed from the pages** —
don't add it back.

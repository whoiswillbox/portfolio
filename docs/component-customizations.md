...
**What:**
- Overlay (scrim): tw-animate `fade-in`/`fade-out` keyed on Radix state (`duration-300` easeOutCubic) so Radix's Presence waits for the fade before unmounting (a plain CSS `transition` got cut off mid-fade = "blocky"). GPU-promoted with `will-change:opacity` + `transform:translate3d(0,0,0)` + `backface-visibility:hidden` + `isolation:isolate` so Safari (in-browser) composites it on a real isolated layer and the alpha interpolates smoothly (it was clean in the PWA but banded in the browser).

...
**What:**
- Overlay (scrim): a fully **opaque** `bg-black` layer whose own **opacity** animates `0.2 → 0` (`data-[state=open]:opacity-20`/`closed:opacity-0`, `transition: opacity 300ms` easeOutCubic). Do NOT use a translucent `bg-black/20` fill — Safari banded the *alpha of a translucent fill* near the iOS status bar / chrome ("blocky" fade-out), but interpolates the *opacity of an opaque layer* smoothly. GPU-promoted to a real isolated Safari compositor layer: `will-change:opacity` + `transform:translate3d(0,0,0)` + `backface-visibility:hidden` + `isolation:isolate`. (Was clean in PWA + desktop, blocky only in mobile Safari near the status bar.)
